...
- 🎚️ **Decisões, um reflexo na frente dos seus modelos (opcional)** — um decisor local (um segundo llama-server compilado do fork parallel-decision do llama.cpp, com um modelo pequeno só para decidir) olha cada mensagem e decide em milissegundos se o modelo local deve pensar e quanto, em vez de um esforço fixo por conversa — sem a mensagem sair da sua máquina. Mensagens simples respondem mais rápido; as difíceis mantêm o orçamento inteiro. A mesma decisão chega aos agentes de código por um proxy local pequeno, que também serve `POST /v1/decision`. O Jev da TypeSafe no OpenRouter é a reserva opcional.
- 🌳 **Modelos Bonsai, com o motor que eles pedem** — os arquivos Ternary Bonsai 2 da PrismML usam tipos de tensor que o llama.cpp padrão recusa. O app lê o cabeçalho do arquivo, instala o fork da PrismML do llama.cpp junto com o download, num clique, e troca o servidor local para ele ao escolher o modelo (e volta à build oficial ao escolher outro).
