...
- 🎚️ **Jev, camada de decisão (opcional)** — um modelo de decisão da TypeSafe, alcançado pela chave do OpenRouter, olha cada mensagem e decide se o modelo local deve pensar e quanto, em vez de um esforço fixo por conversa. Mensagens simples respondem mais rápido; as difíceis mantêm o orçamento inteiro. A mesma decisão chega aos agentes de código por um proxy local pequeno. Vem desligado, e o cartão diz o que sai da sua máquina quando está ligado.
