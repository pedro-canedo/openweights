...
test("one-click optimization presents ready rates and supports restoring", async ({
  ...
  await expect(
    page.getByRole("button", { name: "Usar configuração" }),
  ).toBeVisible();
});

test("cancellation releases the interface without presenting an incomplete result", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Otimizar para meu computador" }),
  ).toBeEnabled();
  await expect(
    page.getByRole("button", { name: "Usar configuração" }),
  ).toHaveCount(0);
...
test("package failures are explained without hiding successful measurements", async ({
  page,
}) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByText(/Algumas opções não puderam ser verificadas/),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Usar configuração" }),
  ).toBeEnabled();
...
test("sem vencedor, a tela nao oferece desfazer o que nao fez", async ({
  page,
}) => {
...
test("o veredito anterior some enquanto uma nova medicao roda", async ({
  page,
}) => {
  await page.goto("/tests/optimization.html");
  await page
...
test("a faster generation option can be applied without a total-time recommendation", async ({
  page,
}) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByText("Configuração em uso", { exact: true }),
  ).toBeVisible();
  expect(
    await page.evaluate(() => (window as any).__applied.at(-1)),
  ).toMatchObject({ armIndex: 1, restore: false });
  await page
    .getByRole("button", { name: "Restaurar configuração anterior" })
    .click();
  await expect(
    page.getByText("Configuração em uso", { exact: true }),
  ).toHaveCount(0);
...
test("history applies an exact saved profile with the server running and refreshes current", async ({
  page,
}) => {
  ...
  expect(
    await page.evaluate(() => (window as any).__applied.at(-1).profile),
  ).toEqual({ ctx: 8192, threads: 16, engine: "official" });
  await expect(
    page.getByText("Perfil não salvo", { exact: true }),
  ).toBeVisible();
});

for (const scenario of ["history-busy", "history-failure"])
  test(`${scenario} never reports an unapplied profile as active`, async ({
    page,
  }) => {
...
test("um perfil editado a mao nao e sobrescrito sem aviso", async ({
  page,
}) => {
  await page.goto("/tests/optimization.html?scenario=hand-edited");
  await page
    .getByRole("button", { name: "Otimizar para meu computador" })
    .click();
  await expect(page.getByRole("radio").first()).toBeVisible();

  // O backend recusa este caso; a tela precisa dizer isso antes do clique, e
  // não oferecer um botão que só falha depois de apertado.
  await expect(
    page.getByText(
      "A configuração mudou desde o teste. Faça uma nova comparação.",
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Usar configuração" }),
