...
(window as any).__TAURI_INTERNALS__ = {
  ...
  async invoke(cmd: string, args: any) {
    ...
    if (cmd === "hardware_profile") return { gpus: [], ramBandwidthBytesS: null };
    if (cmd === "perf_history") return { gpuName: "NVIDIA GeForce RTX 3090", currentProfileKey, bestProfileKey: "current", usage: [], rows: [
      { measuredAt: 1788879600000, genTps: 27.6, promptTps: 1005.8, profileKey: "current", profileSummary: { "gpu-layers": "65", "ctx-size": "65536", "flash-attn": "on", "cache-type-k": "q8_0" }, profile: {ctx:65536, threads:8, engine:"official"}, suspect:false, buildNumber:10441, powerLimitW:260, deltaPct:-14.5, deltaReason:"powerChanged" },
      { measuredAt: 1788879500000, genTps: 33.1, promptTps: 1168.7, profileKey: "alternative", profileSummary: { "gpu-layers": "65", "ctx-size": "8192", "flash-attn": "on" }, profile: {ctx:8192, threads:16, engine:"official"}, suspect:true, buildNumber:10441, powerLimitW:370, deltaPct:null, deltaReason:"suspect" },
      { measuredAt: 1788879400000, genTps: 32.2, promptTps: 1187.4, profileKey: "legacy", profileSummary: { "ctx-size": "32768" }, profile: null, suspect:false, buildNumber:10441, powerLimitW:370, deltaPct:null, deltaReason:"first" }
    ] };
    if (cmd === "tune_apply") {
      (window as any).__applied.push(args);
      if (scenario === "history-busy") throw new Error("engine-busy:external");
      if (scenario === "history-failure") return { ok:false, error:"load-failed", profile:null };
...
const { default: BenchHistoryCard } = await import("../src/components/server/BenchHistoryCard");
createRoot(document.getElementById("root")!).render(<main className="mx-auto max-w-5xl p-4 sm:p-8">{scenario?.startsWith("history") ? <BenchHistoryCard model="Qwopus3-27B-Flash-MTP-Q4_K_M.gguf" running={true} /> : <ComparisonCard model="Qwen test" />}</main>);
