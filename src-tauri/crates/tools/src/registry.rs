...
use crate::{SharedTool, Tool, ToolContext, ToolError, ToolOutput, ToolResult};
...
impl ToolRegistry {
    ...
    pub async fn execute(
        ...
        if let Some((prefix, rest)) = name.split_once(PROVIDER_SEP) {
            if let Some(p) = self.providers.iter().find(|p| p.id() == prefix) {
...
mod tests {
    ...
    async fn routes_builtin_and_provider() {
        ...
        reg.register(Arc::new(Echo)).add_provider(Arc::new(FakeProvider));
    ...
    async fn provider_specs_get_prefix_and_origin() {
        ...
        let mcp = specs.iter().find(|s| s.name.contains("criar_issue")).unwrap();
