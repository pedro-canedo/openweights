...
const WORKLOAD: &str = "interactive-code-v1-temp0-seed42-cacheoff-128";
...
async fn cancellable<T>(future: impl Future<Output = Result<T, String>>) -> Result<T, String> {
    tokio::pin!(future);
    loop {
...
async fn assert_idle(state: &AppState) -> Result<(), String> {
    let cfg = state
        .server
        .lock()
        .await
...
fn runtime_key(cfg: &ServerConfig) -> String {
    // O arquivo e seu mtime invalidam evidência após substituir o runtime.
    let meta = std::fs::metadata(&cfg.exe_path).ok();
    format!(
        "{}:{:?}:{:?}",
...
fn config_key(cfg: &ServerConfig) -> String {
...
async fn sample(
    ...
    let response: Value = client.post(format!("{}/v1/chat/completions", cfg.connect_url()))
        .bearer_auth(cfg.api_key.as_deref().unwrap_or_default())
        .json(&json!({"model": model, "messages": [{"role":"user","content":PROMPT}], "temperature":0, "seed":42, "max_tokens":128, "stream":false, "cache_prompt":false}))
        .send().await.map_err(|e| e.to_string())?.error_for_status().map_err(|e| e.to_string())?
        .json().await.map_err(|e| e.to_string())?;
    let timings = &response["timings"];
    let gen_tps = timings["predicted_per_second"]
        .as_f64()
        .ok_or("comparison-missing-timings")?;
    let prompt_tps = timings["prompt_per_second"]
        .as_f64()
        .ok_or("comparison-missing-timings")?;
    Ok((gen_tps, prompt_tps, start.elapsed().as_secs_f64() * 1000.0))
}
...
async fn arm(
    ...
    let result = async {
        ...
        cancellable(sample(&client, &config, model)).await?; // aquecimento descartado
        ...
        for n in 1..=3 {
            ...
            samples.push(cancellable(sample(&client, &config, model)).await?);
        }
        Ok(Arm {
...
pub async fn compare_run(
    ...
    let measured = async {
        let reference = arm(Some(&app), &config, &model, &original, &entry, 0).await?;
        let proposed = arm(Some(&app), &config, &model, &next, &entry, 1).await?;
...
pub fn compare_latest(
    ...
    if let Some(ref mut item) = saved {
        item.applied = item.arms.get(1).is_some_and(|arm| {
...
pub async fn compare_apply(
    ...
    let proposed = &saved.arms.get(1).ok_or("comparison-missing")?.profile;
...
mod tests {
    ...
    async fn real_isolated_arm_warms_up_measures_and_stops() {
        ...
        let measured = arm(None, &cfg, "test", &profile, &entry, 0).await.unwrap();
